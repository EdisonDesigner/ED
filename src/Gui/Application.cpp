 ...
 * @note In the ED design it is explicitly allowed to have duplicate labels
...
static PyObject *
EDGui_subgraphFromObject(PyObject * /*self*/, PyObject *args)
{
    PyObject *o;
...
static PyObject *
EDGui_getSoDBVersion(PyObject * /*self*/, PyObject *args)
{
...
struct PyMethodDef EDGui_methods[] = {
    {"subgraphFromObject",EDGui_subgraphFromObject,METH_VARARGS,
     "subgraphFromObject(object) -> Node\n\n"
     "Return the Inventor subgraph to an object"},
    {"getSoDBVersion",EDGui_getSoDBVersion,METH_VARARGS,
...
Application::Application(bool GUIenabled)
{
    ...
    if (GUIenabled) {
        ...
        PyObject* module = Py_InitModule3("EDGui", Application::Methods,
            "The functions in the EDGui module allow working with GUI documents,\n"
            "view providers, views, workbenches and much more.\n\n"
            "The EDGui instance provides a list of references of GUI documents which\n"
            ...
            "The EDGui module also provides a set of functions to work with so called\n"
            "workbenches.");
    ...
    Base::PyGILStateLocker lock;
    PyObject *module = PyImport_AddModule("EDGui");
    PyMethodDef *meth = EDGui_methods;
    ...
    if (Module != 0) {
        try {
            ...
            for (std::vector<App::DocumentObject*>::iterator it = sel.begin(); it != sel.end(); ++it) {
                str << "__objs__.append(ED.getDocument(\"" << DocName << "\").getObject(\""
    ...
    if (doc != d->documents.end()) {
        ...
        if (d->activeDocument != doc->second) {
            ...
            if (d->activeDocument) {
                ...
                Py::Module("EDGui").setAttr(std::string("ActiveDocument"),active);
            }
            else {
                Base::PyGILStateLocker lock;
                Py::Module("EDGui").setAttr(std::string("ActiveDocument"),Py::None());
            }
        }
...
#ifdef ED_DEBUG // redirect Coin messages to ED
...
void Application::initApplication(void)
{
    ...
    try {
        initTypes();
        new Base::ScriptProducer( "EDGuiInit", EDGuiInit );
        init_resources();
...
void Application::runApplication(void)
{
...
#if !defined(Q_WS_X11)
    QIcon::setThemeSearchPaths(QIcon::themeSearchPaths() << QString::fromLatin1(":/icons/ED-default"));
    QIcon::setThemeName(QLatin1String("ED-default"));
#endif
    ...
    try {
        ...
        Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("EDGuiInit"));
    }
    catch (const Base::Exception& e) {
        Base::Console().Error("Error in EDGuiInit.py: %s\n", e.what());
        mw.stopSplasher();
        throw;
    }
