...
#include "TaskView/TaskWatcher.h"
...
/** \defgroup workbench Workbench Framework
    ...
    ED provides the possibility to have one or more workbenches for a module.
 ...
 * Once you have implemented your workbench class you have to register it to make it known to the ED core system. You must make sure that the step
 ...
 * your workbench, if not, the default ED icon is taken, and finally the most important method GetClassName(). that represents the link between
 ...
 * \note You must make sure to choose a unique name for your workbench (in this example "My workbench"). Since ED doesn't provide a mechanism for
 ...
 * ED and saves memory.
 * At startup ED scans all module directories and invokes InitGui.py. So an item for a workbench gets created. If the user
 ...
 *
 * ED provides also the possibility to define pure Python workbenches. Such workbenches are temporarily only and are lost after exiting
 * the ED session. But if you want to keep your Python workbench you can write a macro and attach it with a user defined button or just
 * perform the macro during the next ED session.
 ...
 * EDGui.addWorkbench(w)                                 # Creates an item for our workbenmch now
...
MenuItem* StdWorkbench::setupMenuBar() const
{
    ...
    *help << "Std_OnlineHelp" << "Std_EDWebsite"
          << "Std_EDUserHub" << "Std_EDPowerUserHub"
          << "Std_PythonHelp" << "Std_EDForum"
          << "Std_EDFAQ" << "Std_About" << "Std_WhatsThis";
