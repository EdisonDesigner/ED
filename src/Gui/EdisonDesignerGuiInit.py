...
def InitApplications():
	...
	try:
		import pkgutil
		import importlib
		import freecad as edisondesigner
		edisondesigner.gui = FreeCADGui
		for _, edisondesigner_module_name, edisondesigner_module_ispkg in pkgutil.iter_modules(edisondesigner.__path__, "edisondesigner."):
			if edisondesigner_module_ispkg:
				Log('Init: Initializing ' + edisondesigner_module_name + '\n')
				edisondesigner_module = importlib.import_module(edisondesigner_module_name)
				if any (module_name == 'init_gui' for _, module_name, ispkg in pkgutil.iter_modules(edisondesigner_module.__path__)):
					try:
						importlib.import_module(edisondesigner_module_name + '.init_gui')
						Log('Init: Initializing ' + edisondesigner_module_name + '... done\n')
					except ImportError as error:
						Err('During initialization the error ' + str(error) + ' occurred in ' + edisondesigner_module_name + '\n')
				else:
					Log('Init: No init_gui module found in ' + edisondesigner_module_name + ', skipping\n')
