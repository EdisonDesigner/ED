...
PyObject* DocumentPy::setEdit(PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, "s|i;Name of the object to edit has to be given!", &psFeatStr,&mod)) {
        ...
        if (!obj) {
            PyErr_Format(Base::BaseExceptionEdisonDesignerError, "No such object found in document: '%s'", psFeatStr);
            return 0;
        }
...
PyObject*  DocumentPy::mdiViewsOfType(PyObject *args)
{
    ...
    if (type == Base::Type::badType()) {
        PyErr_Format(Base::BaseExceptionEdisonDesignerError, "'%s' is not a valid type", sType);
        return NULL;
    }
