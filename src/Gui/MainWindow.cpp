...
QPixmap MainWindow::splashImage() const
{
	...
	/*std::string splash_dir = App::Application::Config()["AppHomePath"].c_str();
	splash_dir += "data/Gui/";
	QDir dir(QString::fromUtf8(splash_dir.c_str()));*/
	QDir dir(QString::fromUtf8(App::Application::getResourceDir().c_str()));
	QFileInfo fi(dir.filePath(QString::fromLatin1("SplashImg.png")));
	//QDir dir(QString::fromUtf8(App::Application::Config()["BinPath"].c_str()));
    //QFileInfo fi(dir.filePath(QString::fromLatin1("SplashImg.png")));
    ...
    if (tc != App::Application::Config().end()) {
		...
		string test = tc->second.c_str();

		/*QString colorStr = QString::fromUtf8("#ffffff");
        ...
        if (color.isValid()) 
		{
			...
			painter.drawText(w - 120 - v, h - 218, version);
            painter.end();
        }
    }
