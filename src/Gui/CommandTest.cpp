...
{
	sMenuText = "Sci Box";
	sToolTipText = "Easily make science box models";
	sPixmap = "Sci_box";
}
...
{
	sMenuText = "Clamp";
	sToolTipText = "Design clamp model";
	sPixmap = "Clamp";
}
...
{
	sMenuText = "Shaft";
	sToolTipText = "Design shaft model";
	sPixmap = "Shaft";
}

void Sci_Box::activated(int iMsg)
{
	/*QString file = QFileDialog::getOpenFileName(getMainWindow(), QString::fromAscii("Python Open"), QString(),
		QString::fromAscii("Python (*.py)"));

	QDir d = QFileInfo(file).absoluteDir();*/
	std::string homePath = App::Application::Config()["AppHomePath"];
	std::string sciPath = "EDISON_ADDON\\Sci_Box.py";
	std::string filePath = homePath + sciPath;
	//QString file = QString::fromUtf8("D:\\EDISON\\EDISON_ADDON\\Sci_Box.py");
	QString file = QString::fromStdString(filePath);

	if (!file.isNull())
	{
		Application::Instance->macroManager()->run(Gui::MacroManager::File, file.toUtf8());
	}
}

void Clamp::activated(int iMsg)
{
	std::string homePath = App::Application::Config()["AppHomePath"];
	std::string clampPath = "EDISON_ADDON\\clamping.py";
	std::string filePath = homePath + clampPath;
	//QString file = QString::fromUtf8("D:\\EDISON\\EDISON_ADDON\\Sci_Box.py");
	QString file = QString::fromStdString(filePath);
...
void Shaft::activated(int iMsg)
{
	std::string homePath = App::Application::Config()["AppHomePath"];
	std::string shaftPath = "EDISON_ADDON\\shaft.py";
	std::string filePath = homePath + shaftPath;
	//QString file = QString::fromUtf8("D:\\EDISON\\EDISON_ADDON\\Sci_Box.py");
	QString file = QString::fromStdString(filePath);
...
DEF_STD_CMD(Sci_Box_Toolbar)

Sci_Box_Toolbar::Sci_Box_Toolbar()
: Command("Sci_Box_Toolbar")
{
	sMenuText = "Sci_Box";
	sToolTipText = "Easily make science box models";
	sPixmap = "Sci_box";
...
void Sci_Box_Toolbar::activated(int iMsg)
{
	/*QString file = QFileDialog::getOpenFileName(getMainWindow(), QString::fromAscii("Python Open"), QString(),
		QString::fromAscii("Python (*.py)"));*/
...
void CreateTestCommands(void)
{
	...
	rcCmdMgr.addCommand(new Sci_Box());
	rcCmdMgr.addCommand(new Clamp());
	rcCmdMgr.addCommand(new Shaft());

	rcCmdMgr.addCommand(new Sci_Box_Toolbar());
	//rcMmdMgr.addCommand(new Clamp_Toolbar());
	//rcMmdMgr.addCommand(new Shaft_Toolbar());
}
