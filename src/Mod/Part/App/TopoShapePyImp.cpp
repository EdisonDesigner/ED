...
PyObject*  TopoShapePy::__setstate__(PyObject *args) {
    if (! getTopoShapePtr()) {
        PyErr_SetString(Base::BaseExceptionEDError,"no c++ object");
        return 0;
    }
    else {
...
Py::String TopoShapePy::getShapeType(void) const
{
    ...
    if (sh.IsNull())
        throw Py::Exception(Base::BaseExceptionEDError, "cannot determine type of null shape");
...
Py::String TopoShapePy::getOrientation(void) const
{
    TopoDS_Shape sh = getTopoShapePtr()->_Shape;
    if (sh.IsNull())
        throw Py::Exception(Base::BaseExceptionEDError, "cannot determine orientation of null shape");
