...
PyMODINIT_FUNC initPart()
{
    ...
    //
    PyObject* OCCError = 0;
    if (PyObject_IsSubclass(Base::BaseExceptionEDError, PyExc_RuntimeError)) {
        OCCError = PyErr_NewException("Part.OCCError", Base::BaseExceptionEDError, NULL);
    }
    else {
        Base::Console().Error("Can not inherit Part.OCCError form BaseEDError.\n");
