...
PyMODINIT_FUNC initPart()
{
    ...
    //
    PyObject* OCCError = 0;
    if (PyObject_IsSubclass(Base::BaseExceptionEdisonDesignerError, PyExc_RuntimeError)) {
		OCCError = PyErr_NewException("Part.OCCError", Base::BaseExceptionEdisonDesignerError, NULL);
    }
    else {
        Base::Console().Error("Can not inherit Part.OCCError form BaseEdisonDesignerError.\n");
