...
void CrossSections::apply()
{
    ...
    app->runPythonCode("from ED import Base\n");
    for (std::vector<App::DocumentObject*>::iterator it = obj.begin(); it != obj.end(); ++it) {
        App::Document* doc = (*it)->getDocument();
        std::string s = (*it)->getNameInDocument();
        s += "_cs";
        app->runPythonCode(QString::fromLatin1(
            "wires=list()\n"
            "shape=ED.getDocument(\"%1\").%2.Shape\n")
        ...
        app->runPythonCode(QString::fromLatin1(
            ...
            "slice=ED.getDocument(\"%1\").addObject(\"Part::Feature\",\"%2\")\n"
            "slice.Shape=comp\n"
            "slice.purgeTouched()\n"
...
TaskCrossSections::TaskCrossSections(const Base::BoundBox3d& bb)
{
	...
	taskbox = new Gui::TaskView::TaskBox(Gui::BitmapFactory().pixmap("Part_CrossSections"), widget->windowTitle(), true, 0);
