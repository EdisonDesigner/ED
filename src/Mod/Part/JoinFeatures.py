...
# -------------------------- common stuff --------------------------------------------------
def getParamRefine():
    return ED.ParamGet("User parameter:BaseApp/Preferences/Mod/Part/Boolean").GetBool("RefineModel")
...
def makePartJoinFeature(name, mode = 'bypass'):
    ...
    obj = ED.ActiveDocument.addObject("Part::FeaturePython",name)
...
class _ViewProviderPartJoinFeature:
    ...
    def onDelete(self, feature, subelements):
        try:
            ...
            ED.Console.PrintError("Error in onDelete: " + err.message)
        return True
    

def CreateJoinFeature(name, mode):
    sel = EDGui.Selection.getSelectionEx()
    ED.ActiveDocument.openTransaction("Create "+mode+"ObjectsFeature")
    EDGui.addModule("JoinFeatures")
    EDGui.doCommand("j = JoinFeatures.makePartJoinFeature(name = '"+name+"', mode = '"+mode+"' )")
    EDGui.doCommand("j.Base = App.ActiveDocument."+sel[0].Object.Name)
    EDGui.doCommand("j.Tool = App.ActiveDocument."+sel[1].Object.Name)
    try:
        EDGui.doCommand("j.Proxy.execute(j)")
        EDGui.doCommand("j.purgeTouched()")
    except Exception as err:
        ...
        if mb.clickedButton() is btnAbort:
            ED.ActiveDocument.abortTransaction()
            return

    EDGui.doCommand("j.Base.ViewObject.hide()")
    EDGui.doCommand("j.Tool.ViewObject.hide()")
...
class _CommandConnectFeature:
    ...
    def Activated(self):
        if len(EDGui.Selection.getSelectionEx()) == 2 :
    ...
    def IsActive(self):
        if ED.ActiveDocument:
            return True
        else:
            return False
            
EDGui.addCommand('Part_JoinConnect',_CommandConnectFeature())
...
class _CommandEmbedFeature:
    ...
    def Activated(self):
        if len(EDGui.Selection.getSelection()) == 2 :
...
EDGui.addCommand('Part_JoinEmbed',_CommandEmbedFeature())
...
EDGui.addCommand('Part_JoinCutout',_CommandCutoutFeature())
