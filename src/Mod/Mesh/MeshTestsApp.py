...
import ED, os, sys, unittest, Mesh
import thread, time, tempfile, math


#---------------------------------------------------------------------------
# define the functions to test the ED mesh module
#---------------------------------------------------------------------------
...
class PivyTestCases(unittest.TestCase):
	...
	def testRayPick(self):
		...
		from pivy import coin; import EDGui
		Mesh.show(planarMeshObject)
		view=EDGui.ActiveDocument.ActiveView.getViewer()
...
def loadFile(name):
    #lock.acquire()
    mesh=Mesh.Mesh()
    ED.Console.PrintMessage("Create mesh instance\n")
    #lock.release()
    mesh.read(name)
    ED.Console.PrintMessage("Mesh loaded successfully.\n")

def createMesh(r,s):
    ED.Console.PrintMessage("Create sphere (%s,%s)...\n"%(r,s))
    mesh=Mesh.createSphere(r,s)
    ED.Console.PrintMessage("... destroy sphere\n")
...
class LoadMeshInThreadsCases(unittest.TestCase):
    ...
    def testLoadMesh(self):
        ...
        mesh.write(name)
        ED.Console.PrintMessage("Write mesh to %s\n"%(name))
...
class PolynomialFitCases(unittest.TestCase):
    ...
    def testFitGood(self):
        # symmetric
        v=[]
        v.append(ED.Vector(0,0,0.0))
        v.append(ED.Vector(1,0,0.5))
        v.append(ED.Vector(2,0,0.0))
        v.append(ED.Vector(0,1,0.5))
        v.append(ED.Vector(1,1,1.0))
        v.append(ED.Vector(2,1,0.5))
        v.append(ED.Vector(0,2,0.0))
        v.append(ED.Vector(1,2,0.5))
        v.append(ED.Vector(2,2,0.0))
    ...
    def testFitExact(self):
        ...
        v=[]
        v.append(ED.Vector(0,0,0.0))
        v.append(ED.Vector(1,0,0.0))
        v.append(ED.Vector(2,0,0.0))
        v.append(ED.Vector(0,1,0.0))
        v.append(ED.Vector(1,1,1.0))
        v.append(ED.Vector(2,1,0.0))
        d = Mesh.polynomialFit(v)
        c = d["Coefficients"]
        print ("Polynomial: f(x,y)=%f*x^2%+f*y^2%+f*x*y%+f*x%+f*y%+f" % (c[0],c[1],c[2],c[3],c[4],c[5]))
        for i in d["Residuals"]:
