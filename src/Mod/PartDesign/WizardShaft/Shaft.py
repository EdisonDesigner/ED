...
class Shaft:
    ...
    def editConstraint(self,  index):
        if (self.segments[index].constraint is not None):
            EDGui.activeDocument().setEdit(self.segments[index].constraint.Name)
    ...
    def equilibrium(self):
        ...
        try:
            ...
            ED.Console.PrintMessage("numpy is not installed on your system\n")
        ...
        for i in range(len(self.segments)):
            ...
            if cType == "Fixed":
                ...
                if i == 0:
                    ...
                    ED.Console.PrintMessage("Fixed constraint must be at beginning or end of shaft\n")
                    return
                ...
                #ED.Console.PrintMessage("BeltForce1: %f, BeltForce2: %f\n" % (constraint.BeltForce1,  constraint.BeltForce2))
                #ED.Console.PrintMessage("Angle1: %f, Angle2: %f\n" % (forceAngle1,  forceAngle2))
        ...
        for ax in range(3):
            ED.Console.PrintMessage("Axis: %u\n" %  ax) 
            ...
            if  len(coefficientsF[ax]) <= 1:
                ...
                ED.Console.PrintMessage("Matrix is singular, no solution possible\n")
            ...
            if len(coefficientsF[ax]) == 2:
                ...
                if coefficientsM[ax][1] != 0.0 and coefficientsM[ax][0] != 0.0:
                    ...
                    if abs(solution[0] - solution[1]) < 1E9:
                        ED.Console.PrintMessage("System is statically undetermined. No solution possible.\n")
                        self.parent.updateButtons(ax,  False)
                        continue
            else:            
               ...
               # TODO: This could easily be done manually... there are only 2 variables and 6 coefficients
                ...
                try:
                    ...
                    ED.Console.PrintMessage(e.message)
                    ED.Console.PrintMessage(". No solution possible.\n")
                    self.parent.updateButtons(ax,  False)
                    continue
            ...
            for i in range(2):
                if solution[i] is None:
                    continue
                ED.Console.PrintMessage("Reaction force/moment: %s = %f\n" % (variableNames[ax][i+1],  solution[i]))
            ...
            ED.Console.PrintMessage(forces[ax])
            ED.Console.PrintMessage("\n")
            ED.Console.PrintMessage(moments[ax])
            ED.Console.PrintMessage("\n")
    ...
    def printEquilibrium(self, var, coeff):
        ...
        for i in range(len(var)):
            if i == 0:
                ED.Console.PrintMessage("%f = " % coeff[i])
            else:
                ED.Console.PrintMessage("%f * %s" % (coeff[i], var[i]))
