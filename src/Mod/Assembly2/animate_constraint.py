...
class AnimateConstraint_Command:
    def Activated(self):
        ...
        if not 'ConstraintInfo' in constraint_to_animate.Content: #then mirror selected
            constraint_to_animate = ED.ActiveDocument.getObject( constraint_to_animate.ViewObject.Proxy.constraintObj_name )
            #return
...
EDGui.addCommand( 'assemly2_animate_constraint', AnimateConstraint_Command() )
...
class AnimateConstraint_TaskPanel:
    ...
    def reject(self): #or more correctly close, given the button settings
        if hasattr( self.form, 'constraintAnimator' ):
            ...
            solveConstraints( ED.ActiveDocument )
        ...
        try:
            ...
            for count, a in enumerate(A):
                if not p.wasCanceled():
                    ...
                    ED.ActiveDocument.recompute() #update other expersions
                    constraint_sys = solveConstraints( ED.ActiveDocument )
...
class ConstraintAnimator:
    ...
    def showAt( self, time ):
        ...
        self.variableManager.updateEDValues( self.X[frame] )
        return frame
...
class RealParemeter:
    ...
    def add_property_to_ED_object( self ):
