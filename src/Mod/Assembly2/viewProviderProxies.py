...
def allow_deletetion_when_activice_doc_ne_object_doc():
    parms = ED.ParamGet("User parameter:BaseApp/Preferences/Mod/Assembly2")
...
class ImportedPartViewProviderProxy:
    def onDelete(self, viewObject, subelements): # subelements is a tuple of strings
        if not allow_deletetion_when_activice_doc_ne_object_doc() and ED.activeDocument() != viewObject.Object.Document:
            ED.Console.PrintMessage("preventing deletetion of %s since active document != %s. Disable behavior in assembly2 preferences.\n" % (viewObject.Object.Label, viewObject.Object.Document.Name) )
            return False
        obj = viewObject.Object
        doc = obj.Document
        #ED.Console.PrintMessage('ConstraintObjectViewProviderProxy.onDelete: removing constraints refering to %s (label:%s)\n' % (obj.Name, obj.Label))
        deleteList = []
        for c in doc.Objects:
            if 'ConstraintInfo' in c.Content:
                if obj.Name in [ c.Object1, c.Object2 ]:
                    deleteList.append(c)
        if len(deleteList) > 0:
            #ED.Console.PrintMessage("  delete list %s\n" % str(deleteList) )
            for c in deleteList:
                #ED.Console.PrintMessage("  - removing constraint %s\n" % c.Name )
    ...
    def claimChildren(self):
        ...
        #    importedPart = ED.ActiveDocument.getObject( self.object_Name )
    ...
    def setupContextMenu(self, ViewObject, popup_menu):
        ...
        #if self.Object.Document == ED.ActiveDocument:
        #    for label, cmd in [
...
class PopUpMenuItem:
    def __init__( self, proxy, menu, label, ED_cmd ):
...
def create_constraint_mirror( constraintObj, iconPath, origLabel= '', mirrorLabel='', extraLabel = '' ):
    #ED.Console.PrintMessage("creating constraint mirror\n")
...
class ConstraintMirrorObjectProxy:
    ...
    def onChanged(self, obj, prop):
        ...
        '''
        #ED.Console.PrintMessage("%s.%s property changed\n" % (obj.Name, prop))
    ...
    for m in matches:
        ...
        def get_node_by_label( label ):
            if label in tree_nodes and len( tree_nodes[label] ) == 1:
                ...
                ED.Console.PrintWarning( "  repair_tree_view: skipping %s since no node with text(0) == %s\n" % ( label, label) )
            else:
                ED.Console.PrintWarning( "  repair_tree_view: skipping %s since multiple nodes matching label\n" % ( label, label) )
        if doc.Label in tree_nodes: #all the code up until now has geen to find the QtGui.QTreeView widget (except for the get_node_by_label function)
            #ED.Console.PrintMessage( tree_nodes )
            for imported_obj in doc.Objects:
                try: #allow use of assembly2 contraints also on non imported objects
                    if isinstance( imported_obj.ViewObject.Proxy, ImportedPartViewProviderProxy ):
                        #ED.Console.PrintMessage( 'checking claim children for %s\n' % imported_obj.Label )
                        if get_node_by_label( imported_obj.Label ):
                            ...
                            if not hasattr( imported_obj.ViewObject.Proxy, 'Object'):
                                ...
                                ED.Console.PrintMessage('repair_tree_view: %s.ViewObject.Proxy.Object = %s' % (imported_obj.Name, imported_obj.Name) )
                    ...
                    # ED.Console.PrintWarning( "not repaired %s \n" % ( imported_obj.Label ) )
                    pass
            #break
