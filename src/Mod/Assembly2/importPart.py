...
if __name__ == '__main__': #then testing library.
    ...
    assert not hasattr(EDGui, 'addCommand')
    EDGui.addCommand = lambda x,y: 0
...
def importPart( filename, partName=None, doc_assembly=None ):
    if doc_assembly == None:
        doc_assembly = ED.ActiveDocument
    updateExistingPart = partName != None
    if updateExistingPart:
        ED.Console.PrintMessage("updating part %s from %s\n" % (partName,filename))
    else:
        ED.Console.PrintMessage("importing part from %s\n" % filename)
    doc_already_open = filename in [ d.FileName for d in ED.listDocuments().values() ]
    debugPrint(4, "%s open already %s" % (filename, doc_already_open))
    if doc_already_open:
        doc = [ d for d in ED.listDocuments().values() if d.FileName == filename][0]
    else:
        if filename.lower().endswith('.fcstd'):
            debugPrint(4, '  opening %s' % filename)
            doc = ED.openDocument(filename)
            ...
            import ImportGui
            doc = ED.newDocument( os.path.basename(filename) )
    ...
    if any([ 'importPart' in obj.Content for obj in doc.Objects]) and not len(visibleObjects) == 1:
        ...
        if len(visibleObjects) != 1:
            if not updateExistingPart:
                msg = "A part can only be imported from a EdisonDesigner document with exactly one visible part. Aborting operation"
                QtGui.QMessageBox.information(  QtGui.qApp.activeWindow(), "Value Error", msg )
            else:
                msg = "Error updating part from %s: A part can only be imported from a EdisonDesigner document with exactly one visible part. Aborting update of %s" % (partName, filename)
            QtGui.QMessageBox.information(  QtGui.qApp.activeWindow(), "Value Error", msg )
    ...
    if not doc_already_open: #then close again
        ED.closeDocument(doc.Name)
        ED.setActiveDocument(doc_assembly.Name)
        ED.ActiveDocument = doc_assembly
    return obj
...
class ImportPartCommand:
    def Activated(self):
        if EDGui.ActiveDocument == None:
            ED.newDocument()
        view = EDGui.activeDocument().activeView()
        #filename, filetype = QtGui.QFileDialog.getOpenFileName(
        #    QtGui.qApp.activeWindow(),
        #    "Select EdisonDesigner document to import part from",
        #    "",# "" is the default, os.path.dirname(ED.ActiveDocument.FileName),
        #    "EdisonDesigner Document (*.fcstd)"
        #    )
    ...
    def GetResources(self):
        return {
            ...
            'MenuText': 'Import a part from another EdisonDesigner document',
            'ToolTip': 'Import a part from another EdisonDesigner document'
            }
EDGui.addCommand('importPart', ImportPartCommand())
...
def path_rel_to_abs(path):
    j = ED.ActiveDocument.FileName.rfind('/')
    k = path.find('/')
    absPath = ED.ActiveDocument.FileName[:j] + path[k:]
    ED.Console.PrintMessage("First %s\n" % ED.ActiveDocument.FileName[:j])
    ED.Console.PrintMessage("Next %s\n" % path[k:])
    ED.Console.PrintMessage("absolutePath is %s\n" % absPath)
...
EDGui.addCommand('updateImportedPartsCommand', UpdateImportedPartsCommand())
...
def duplicateImportedPart( part ):
    ...
    while nameBase[-1] in '0123456789' and len(nameBase) > 0:
        nameBase = nameBase[:-1]
    try:
        newObj = ED.ActiveDocument.addObject("Part::FeaturePython", findUnusedObjectName(nameBase))
    except UnicodeEncodeError:
        safeName = findUnusedObjectName('import_')
        newObj = ED.ActiveDocument.addObject("Part::FeaturePython", safeName)
...
class MovePartCommand:
    def Activated(self):
        ...
        if len(selection) == 1:
            PartMover(  EDGui.activeDocument().activeView(), selection[0].Object )
        else:
...
EDGui.addCommand('assembly2_movePart', MovePartCommand())

class DuplicatePartCommand:
    def Activated(self):
        selection = [s for s in EDGui.Selection.getSelectionEx() if s.Document == ED.ActiveDocument ]
        if len(selection) == 1:
            PartMover(  EDGui.activeDocument().activeView(), duplicateImportedPart( selection[0].Object ) )
...
EDGui.addCommand('assembly2_duplicatePart', DuplicatePartCommand())
...
class EditPartCommand:
    def Activated(self):
        selection = [s for s in EDGui.Selection.getSelection() if s.Document == ED.ActiveDocument ]
        obj = selection[0]
        docs = ED.listDocuments().values()
        docFilenames = [ d.FileName for d in docs ]
        if not obj.sourceFile in docFilenames :
            ED.open(obj.sourceFile)
            ...
            ED.setActiveDocument( name )
            ED.ActiveDocument=ED.getDocument( name )
            EDGui.ActiveDocument=EDGui.getDocument( name )
    def GetResources(self):
        return {
            'MenuText': 'edit',
            }
EDGui.addCommand('assembly2_editImportedPart', EditPartCommand())
...
class ForkPartCommand:
    def Activated(self):
        ...
        filename, filetype = QtGui.QFileDialog.getSaveFileName(
            ...
            os.path.dirname(ED.ActiveDocument.FileName),
            "EdisonDesigner Document (*.fcstd)"
            )
        if filename == '':
            return
...
EDGui.addCommand('assembly2_forkImportedPart', ForkPartCommand())
...
class DeletePartsConstraints:
    def Activated(self):
        ...
        part = selection[0]
        deleteList = []
        for c in ED.ActiveDocument.Objects:
...
EDGui.addCommand('assembly2_deletePartsConstraints', DeletePartsConstraints())
...
def importUpdateConstraintSubobjects( doc, oldObject, newObject ):
    '''
    TO DO (if time allows): add a task dialog (using EDGui.Control.addDialog) as to allow the user to specify which scheme to use to update the constraint subelement names.
    '''
