        ...
        if 'ConstraintInfo' in obj.Content:
            if not (obj.Object1 in objectNames and obj.Object2 in objectNames):
                ...
                if response == QtGui.QMessageBox.Yes:
                    ED.Console.PrintError("removing constraint %s" % obj.Name)
                    doc.removeObject(obj.Name)
                else:
                    missingObject = obj.Object2 if obj.Object1 in objectNames else obj.Object1
                    ED.Console.PrintError("aborted solving constraints due to %s refering the non-existent object %s" % (obj.Name, missingObject))
                    return False
    return True
...
def solveConstraints( doc, showFailureErrorDialog=True, printErrors=True, cache=None ):
    ...
    for constraintObj in constraintObjectQue[que_start:]:
        ...
        try:
            ...
            if printErrors:
                ED.Console.PrintError('UNABLE TO SOLVE CONSTRAINTS! info:')
                ED.Console.PrintError(e)
            solved = False
            break
        except:
            if printErrors:
    ...
    if solved:
        ...
        t_update_ED_start = time.time()
        variableManager.updateEDValues( constraintSystem.variableManager.X )
        debugPrint( 4,'  time to update EdisonDesigner placement variables %3.2fs' % (time.time()-t_update_ED_start) )
...
EDGui.addCommand('assembly2SolveConstraints', Assembly2SolveConstraintsCommand())
