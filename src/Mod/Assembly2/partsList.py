...
class AddPartsList:
    def Activated(self):
        if not drawing_dimensioning_installed:
            QtGui.QMessageBox.critical( QtGui.qApp.activeWindow(), 'drawing dimensioning wb required', 'the parts list feature requires the drawing dimensioning wb (https://github.com/hamish2014/EDrawing_dimensioning). Release from 12 April 2016 or later required.' )
            return
        ...
        P = PartsList()
        for obj in ED.ActiveDocument.Objects:
        ...
        EDGui.Control.showDialog( d.taskDialog )
...
EDGui.addCommand('addPartsList', AddPartsList())


class PartsListTaskDialog:
    def __init__(self):
        from assembly2lib import __dir__
        self.form = EDGui.PySideUic.loadUi( ':/assembly2/ui/partsList.ui' )
    ...
    def setIntialValues(self):
        parms = ED.ParamGet("User parameter:BaseApp/Preferences/Mod/Assembly2/partsList")
        form = self.form
