...
class CheckAssemblyCommand:
    def Activated(self):
        debugPrint(2, 'Conducting Assembly Part Overlap Check for: %s' % ED.ActiveDocument.Label)
        objects = [obj for obj in ED.ActiveDocument.Objects if hasattr(obj, 'Shape') and obj.Name != 'muxedAssembly']
        n = len(objects)
                ...
                if not p.wasCanceled():
                    ...
                    if boundBoxesOverlap(objects[i].Shape, objects[j].Shape,  tol = 10**-5 ): #first do a rough check, to speed up checks, on the test case used, time reduce from 11s ->10s ...
                        try:
                            ...
                            ED.Console.PrintError('Unable to perform %s:\n' % msg)
...
EDGui.addCommand('assembly2_checkAssembly', CheckAssemblyCommand())
