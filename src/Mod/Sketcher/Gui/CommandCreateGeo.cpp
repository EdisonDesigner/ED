...
public:
	...
	void setupUi(QDialog *Gui__Dialog__AboutApplication, QString xxx, QString yyy)
	{
		if (Gui__Dialog__AboutApplication->objectName().isEmpty())
			Gui__Dialog__AboutApplication->setObjectName(QString::fromUtf8("SketcherGui__SketchOrientationDialog"));
		Gui__Dialog__AboutApplication->resize(178, 121);
		verticalLayout = new QVBoxLayout(Gui__Dialog__AboutApplication);
		verticalLayout->setObjectName(QString::fromUtf8("verticalLayout"));
		gridLayout = new QGridLayout();
		gridLayout->setObjectName(QString::fromUtf8("gridLayout"));

		label_1 = new QLabel(Gui__Dialog__AboutApplication);
		label_1->setObjectName(QString::fromUtf8("label_1"));

		gridLayout->addWidget(label_1, 0, 0, 1, 1);

		name_1 = new QLineEdit(Gui__Dialog__AboutApplication);
		name_1->setObjectName(QString::fromUtf8("name_1"));

		name_1->setText(xxx);
		gridLayout->addWidget(name_1, 0, 1, 1, 1);

		label_2 = new QLabel(Gui__Dialog__AboutApplication);
		label_2->setObjectName(QString::fromUtf8("label_2"));

		gridLayout->addWidget(label_2, 1, 0, 1, 1);

		name_2 = new QLineEdit(Gui__Dialog__AboutApplication);
		name_2->setObjectName(QString::fromUtf8("name_2"));
		name_2->setText(yyy);
		gridLayout->addWidget(name_2, 1, 1, 1, 1);


		verticalLayout->addLayout(gridLayout);

		buttonBox = new QDialogButtonBox(Gui__Dialog__AboutApplication);
		...
		QObject::connect(buttonBox, SIGNAL(accepted()), Gui__Dialog__AboutApplication, SLOT(accept()));
		QObject::connect(buttonBox, SIGNAL(rejected()), Gui__Dialog__AboutApplication, SLOT(reject()));
	...
	{
		Gui__Dialog__AboutApplication->setWindowTitle(QApplication::translate("SketcherGui::InsertDatum", "Point Add", 0, QApplication::UnicodeUTF8));

		label_1->setText(QApplication::translate("SketcherGui::InsertDatum", "X", 0, QApplication::UnicodeUTF8));
		label_2->setText(QApplication::translate("SketcherGui::InsertDatum", "Y", 0, QApplication::UnicodeUTF8));
	} // retranslateUi


};
//////////////// DetailXY 좌표 입력창 생성끝 cuber


//////////////// DetailXY slot 좌표 입력창 생성 cuber
class DetailXY_slot{
public:
	QVBoxLayout *verticalLayout;
	QGridLayout *gridLayout;
	...
	void retranslateUi(QDialog *Gui__Dialog__AboutApplication)
	{
		Gui__Dialog__AboutApplication->setWindowTitle(QApplication::translate("SketcherGui::InsertDatum", "Slot Parameter", 0, QApplication::UnicodeUTF8));

		label_1->setText(QApplication::translate("SketcherGui::InsertDatum", "R", 0, QApplication::UnicodeUTF8));
		label_2->setText(QApplication::translate("SketcherGui::InsertDatum", "L", 0, QApplication::UnicodeUTF8));
	} // retranslateUi


};
//////////////// DetailXY slot 좌표 입력창 생성끝 cuber
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		...
		// 라인 생성 시 두번 째 포인트 클릭 시 좌표입력창 띄우기 cuber
		if (Mode == STATUS_SEEK_First){
			...
			xx.setNum(onSketchPos.fX);
			yy.setNum(onSketchPos.fY);
			deXY.setupUi(&dlg, xx, yy);

			if (dlg.exec() == QDialog::Accepted){
				onSketchPos.fX = deXY.name_1->text().toDouble();
				onSketchPos.fY = deXY.name_2->text().toDouble();

				EditCurve[0] = onSketchPos;
				Mode = STATUS_SEEK_Second;//onSketchPos = onSketchPos;
			}
		}
		else {
			EditCurve[1] = onSketchPos;
		...
		return true;
		// 라인 생성 시 두번째 포인트 클릭 시 좌표입력창 띄우기 끝 cuber


		/* 라인 생성 시 좌표입력창 띄우기 원본 cuber
		if (Mode==STATUS_SEEK_First){
		EditCurve[0] = onSketchPos;
	...
	virtual bool releaseButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_End){
			...
			try {
				// 라인 생성 시 첫번째 포인트 클릭 시 좌표입력창 띄우기 cuber
				DetailXY deXY;
				QDialog dlg(Gui::getMainWindow());
				QString xx;
				QString yy;
				...
				if (dlg.exec() == QDialog::Accepted){
					//onSketchPos = onSketchPos;
					EditCurve[1].fX = deXY.name_1->text().toDouble();
					EditCurve[1].fY = deXY.name_2->text().toDouble();
				...
				// 라인 생성 시 첫번째 포인트 클릭 시 좌표입력창 띄우기 끝 cuber

				/* 라인 생성 시 첫번째 포인트 클릭 시 좌표입력창 띄우기 원본 cuber
				Gui::Command::openCommand("Add sketch line");
				Gui::Command::doCommand(Gui::Command::Doc,"App.ActiveDocument.%s.addGeometry(Part.Line(App.Vector(%f,%f,0),App.Vector(%f,%f,0)),%s)",
				sketchgui->getObject()->getNameInDocument(),
				EditCurve[0].fX,EditCurve[0].fY,EditCurve[1].fX,EditCurve[1].fY,
				...
				Gui::Command::commitCommand();
				라인 생성 시 첫번째 포인트 클릭 시 좌표입력창 띄우기 원본 끝 cuber */
			}
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_SEEK_First){
			...
			QString xx;
			QString yy;
			xx.setNum(EditCurve[0].fX);
			yy.setNum(EditCurve[0].fY);
			deXY.setupUi(&dlg, xx, yy);
			if (dlg.exec() == QDialog::Accepted){
				EditCurve[0].fX = deXY.name_1->text().toDouble();
				EditCurve[0].fY = deXY.name_2->text().toDouble();

				EditCurve[4].fX = deXY.name_1->text().toDouble();
				EditCurve[4].fY = deXY.name_2->text().toDouble();
				Mode = STATUS_SEEK_Second;
			}
			// 직사각형 첫 좌표 입력 창 끝 cuber
	...
	virtual bool releaseButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_End){
			...
			try {
				...
				//직사각형 두번째 좌표 입력창 cuber
				DetailXY deXY;
				QDialog dlg(Gui::getMainWindow());
				QString xx;
				QString yy;
				xx.setNum(EditCurve[2].fX);
				yy.setNum(EditCurve[2].fY);
				deXY.setupUi(&dlg, xx, yy);
				if (dlg.exec() == QDialog::Accepted){
					EditCurve[2].fX = deXY.name_1->text().toDouble();
					EditCurve[2].fY = deXY.name_2->text().toDouble();
				...
				//직사각형 두번째 좌표 입력창 끝 cuber



				/*
				직사각형 두번째 좌표 입력창 원본 cuber
				Gui::Command::openCommand("Add sketch box");
				Gui::Command::doCommand(Gui::Command::Doc,
				"geoList = []\n"
				"geoList.append(Part.Line(App.Vector(%f,%f,0),App.Vector(%f,%f,0)))\n"
	...
	virtual bool releaseButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_Do || Mode == STATUS_Close) {
			...
			if (SegmentMode == SEGMENT_MODE_Line) {
				// issue the geometry
				try {
					// 폴리라인 두번째 부터 좌표 생성 창 cuber
					DetailXY deXY;
					QDialog dlg(Gui::getMainWindow());
					QString xx;
					QString yy;
					xx.setNum(EditCurve[1].fX);
					yy.setNum(EditCurve[1].fY);
					deXY.setupUi(&dlg, xx, yy);
					if (dlg.exec() == QDialog::Accepted){
						EditCurve[1].fX = deXY.name_1->text().toDouble();
						EditCurve[1].fY = deXY.name_2->text().toDouble();
					...
					// 폴리라인 두번째 부터 좌표 생성 창 끝 cuber

					/*
					폴리라인 두번째 부터. 원본 cuber
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		...
		if (Mode == STATUS_SEEK_First){
			...
			QString xx;
			QString yy;
			xx.setNum(CenterPoint.fX);
			yy.setNum(CenterPoint.fY);
			deXY.setupUi(&dlg, xx, yy);
			if (dlg.exec() == QDialog::Accepted){
				CenterPoint.fX = deXY.name_1->text().toDouble();
				CenterPoint.fY = deXY.name_2->text().toDouble();
				EditCurve.resize(34);
				EditCurve[0].fX = CenterPoint.fX;
				EditCurve[0].fY = CenterPoint.fY;
				Mode = STATUS_SEEK_Second;
			}
			//아크 첫 점만 좌표 생성 끝 cuber
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		// 호 첫점(end point end rim point) cuber 
		if (Mode == STATUS_SEEK_First){
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_SEEK_First){
			...
			// 타원 세번째 항목 좌표 입력창 끝 cuber

			/* 타원 세번째 항목 원본 cuber
			EditCurve[0] = onSketchPos;
			centerPoint = onSketchPos;
			Mode = STATUS_SEEK_Second;
			타원 세번째 항목 원본 끝 cuber
			*/
		}
		else if (Mode == STATUS_SEEK_Second) {
			EditCurve[1] = onSketchPos;
...
public:
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_SEEK_First) {
			...
			QString xx;
			QString yy;
			xx.setNum(FirstPoint.fX);
			yy.setNum(FirstPoint.fY);
			deXY.setupUi(&dlg, xx, yy);
			if (dlg.exec() == QDialog::Accepted){
				EditCurve.resize(N + 2);

				FirstPoint.fX = deXY.name_1->text().toDouble();
				FirstPoint.fY = deXY.name_2->text().toDouble();
...
public:
	...
	virtual bool releaseButton(Base::Vector2D onSketchPos)
	{
		if (selectionDone){
			...
			//포인트 좌표 생성 시 좌표 입력창 띄우기 cuber
			try {
				DetailXY deXY;
				QDialog dlg(Gui::getMainWindow());
				QString xx;
				QString yy;
				xx.setNum(EditPoint.fX);
				yy.setNum(EditPoint.fY);
				deXY.setupUi(&dlg, xx, yy);
				if (dlg.exec() == QDialog::Accepted){
					Gui::Command::openCommand("Add sketch point");
					Gui::Command::doCommand(Gui::Command::Doc, "App.ActiveDocument.%s.addGeometry(Part.Point(App.Vector(%f,%f,0)))",
						sketchgui->getObject()->getNameInDocument(),
						deXY.name_1->text().toDouble(), deXY.name_2->text().toDouble());
					Gui::Command::commitCommand();
				}
			}
			catch (const Base::Exception& e) {
			...
			//포인트 좌표 생성 시 좌표 입력창 띄우기 끝 cuber

			/*
			//포인트 좌표 생성 시 좌표 입력창 띄우기 원본 cuber
			try {
			Gui::Command::openCommand("Add sketch point");
			Gui::Command::doCommand(Gui::Command::Doc,"App.ActiveDocument.%s.addGeometry(Part.Point(App.Vector(%f,%f,0)))",
			sketchgui->getObject()->getNameInDocument(),
			...
			//포인트 좌표 생성 시 좌표 입력창 띄우기 원본 끝 cuber */
...
public:
	...
	virtual void mouseMove(Base::Vector2D onSketchPos)
	{
		...
		if (Mode == STATUS_SEEK_First) {
			...
			//세로일때 거리 안뜨는 문제가 있어서 if문 추가 cuber
			if (ly == 0){
				text.sprintf(" (%.1fR %.1fL)", r, lx);
			}
			else{
				text.sprintf(" (%.1fR %.1fL)", r, ly);
			}
			// 추가 끝 cuber
	...
	virtual bool pressButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_SEEK_First){
			...
			QString xx;
			QString yy;
			xx.setNum(StartPos.fX);
			yy.setNum(StartPos.fY);
			deXY.setupUi(&dlg, xx, yy);
			if (dlg.exec() == QDialog::Accepted){
				StartPos.fX = deXY.name_1->text().toDouble();
				StartPos.fY = deXY.name_2->text().toDouble();
				Mode = STATUS_SEEK_Second;
			}
			//타원 세번째 항목 좌표 입력 cuber
	...
	virtual bool releaseButton(Base::Vector2D onSketchPos)
	{
		if (Mode == STATUS_End){
			...
			try {
				...
				deXY.setupUi(&dlg, xx, yy);
				if (dlg.exec() == QDialog::Accepted){
					r = deXY.name_1->text().toDouble();

					if (lx == 0){
						ly = deXY.name_2->text().toDouble();
					}
					else{
						lx = deXY.name_2->text().toDouble();
					}
