...
//////////////// DetailXY2 좌표 입력창 띄우기 필수파일 추가부분 cuber
...
//////////////// DetailXY2 좌표 입력창 띄우기 필수파일 추가부분 끝 cuber
...
public:
	...
	{
		Gui__Dialog__AboutApplication->setWindowTitle(QApplication::translate("SketcherGui::InsertDatum", "Point Move", 0, QApplication::UnicodeUTF8));
	...
	if (Button == 1) {
		if (pressed) {
			...
			case STATUS_SKETCH_DragPoint:
						...
						if (GeoId != Sketcher::Constraint::GeoUndef && PosId != Sketcher::none) {
							...
							try {
								Gui::Command::doCommand(Gui::Command::Doc, "App.ActiveDocument.%s.movePoint(%i,%i,App.Vector(%f,%f,0),%i)"
									...
									, GeoId, PosId, deXY.name_1->text().toDouble(), deXY.name_2->text().toDouble(), relative ? 1 : 0
									);
				...
				////포인트 이동 시 좌표 입력창 띄우기 끝 cuber

				/* 포인트 이동 시 좌표 입력창 띄우기 원본 cuber
				if (edit->DragPoint != -1) {
				int GeoId;
				Sketcher::PointPos PosId;
				getSketchObject()->getGeoVertexIndex(edit->DragPoint, GeoId, PosId);
				...
				Mode = STATUS_NONE;
				return true;
				포인트 이동 시 좌표 입력창 띄우기 원본 끝 cuber*/
