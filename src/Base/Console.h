 ...
 *  This class distribute the Messages issued to the EDConsole class. 
 ...
 *  Messages are distributed with the EDConsoleObserver. The
 *  EDConsole class itself makes no IO, it's more like a manager.
 *  \par
...
public:
    ...
    /// Attaches an Observer to EDConsole
    void AttachObserver(ConsoleObserver *pcObserver);
    /// Detaches an Observer from EDConsole
    ...
    /// Enables or disables message types of a cetain console observer
    bool IsMsgTypeEnabled(const char* sObs, ED_ConsoleMsgType type) const;
