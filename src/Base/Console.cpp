    ...
    PY_TRY{
        ...
        if(pObs)
        {
            if(strcmp(pstr2,"Log") == 0)
				...
				Py_Error(Base::BaseExceptionEdisonDesignerError, "Unknown Message Type (use Log,Err,Msg or Wrn)");

            Py_INCREF(Py_None);
            return Py_None;
        } else {
			Py_Error(Base::BaseExceptionEdisonDesignerError, "Unknown Console Type");
    }
