...
/**
 * \a type can be OR'ed with any of the ED_ConsoleMsgType flags to enable -- if \a b is true --
...
bool ConsoleSingleton::IsMsgTypeEnabled(const char* sObs, ED_ConsoleMsgType type) const
{
    ConsoleObserver* pObs = Get(sObs);
    if (pObs) {
        switch (type) {
 ...
 *  case ED running with GUI a Message in the status Bar apear. In console
 ...
 *  case ED running with GUI a Message Box is poping up. In console
 ...
 *  case ED running with GUI a Error Message Box is poping up. In console
    ...
    PY_TRY{
        ...
        if(pObs)
        {
            if(strcmp(pstr2,"Log") == 0)
                ...
                Py_Error(Base::BaseExceptionEDError,"Unknown Message Type (use Log,Err,Msg or Wrn)");

            Py_INCREF(Py_None);
            return Py_None;
        } else {
            Py_Error(Base::BaseExceptionEDError,"Unknown Console Type");                     
    }
