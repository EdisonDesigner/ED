...
public:
    ...
    void visit(Expression * node) {
        VariableExpression *expr = ED_dynamic_cast<VariableExpression>(node);
...
void PropertyExpressionEngine::Restore(Base::XMLReader &reader)
{
    ...
    for (int i = 0; i < count; ++i) {
        DocumentObject * docObj = ED_dynamic_cast<DocumentObject>(getContainer());
...
{
#ifdef ED_PROPERTYEXPRESSIONENGINE_LOG
...
DocumentObjectExecReturn *App::PropertyExpressionEngine::execute()
{
    ...
    while (it != evaluationOrder.end()) {
        ...
        DocumentObject* parent = ED_dynamic_cast<DocumentObject>(prop->getContainer());
...
{
    DocumentObject * owner = ED_dynamic_cast<DocumentObject>(getContainer());
