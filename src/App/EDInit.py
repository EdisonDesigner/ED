...
# Modified by Youngki Kim on 20 Aug 2017
#
...
def InitApplications():
	try:
		...
		ED.Console.PrintError("\n\nSeems the python standard libs are not installed, bailing out!\n\n")
		raise
	# Checking on ED module path ++++++++++++++++++++++++++++++++++++++++++
	ModDir = ED.getHomePath()+'Mod'
	ModDir = os.path.realpath(ModDir)
	BinDir = ED.getHomePath()+'bin'
	BinDir = os.path.realpath(BinDir)
	LibDir = ED.getHomePath()+'lib'
	LibDir = os.path.realpath(LibDir)
	Lib64Dir = ED.getHomePath()+'lib64'
	Lib64Dir = os.path.realpath(Lib64Dir)
	AddPath = ED.ConfigGet("AdditionalModulePaths").split(";")
	HomeMod = ED.ConfigGet("UserAppData")+"Mod"
	HomeMod = os.path.realpath(HomeMod)
	MacroDir = ED.ParamGet("User parameter:BaseApp/Preferences/Macro").GetString("MacroPath")
	MacroMod = os.path.realpath(MacroDir+"/Mod")
	SystemWideMacroDir = ED.getHomePath()+'Macro'
	SystemWideMacroDir = os.path.realpath(SystemWideMacroDir)

	#print ED.getHomePath()
	if os.path.isdir(ED.getHomePath()+'src\\Tools'):
		sys.path.append(ED.getHomePath()+'src\\Tools')
	...
	if len(platform.mac_ver()[0]) > 0:
		sys.path.append(os.path.expanduser('~/Library/Application Support/ED/Mod'))

# some often used shortcuts (for lazy people like me  ;-)
App = ED
Log = ED.Console.PrintLog
Msg = ED.Console.PrintMessage
Err = ED.Console.PrintError
Wrn = ED.Console.PrintWarning
test_ascii = lambda s: all(ord(c) < 128 for c in s)

Log ('Init: starting App::EDInit.py\n')

# init every application by importing Init.py
InitApplications()

ED.addImportType("ED document (*.FCStd)","ED")
