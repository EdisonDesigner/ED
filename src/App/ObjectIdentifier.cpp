...
/// Here the ED includes sorted by Base,App,Gui......
#include "Property.h"
...
{
    if (owner) {
        const DocumentObject * docObj = ED_dynamic_cast<const DocumentObject>(owner);
        if (!docObj)
...
ObjectIdentifier::ObjectIdentifier(const Property &prop)
    ...
    , documentObjectNameSet(false)
{
    DocumentObject * docObj = ED_dynamic_cast<DocumentObject>(prop.getContainer());
...
{
    if (ED_dynamic_cast<DocumentObject>(owner) == 0)
        return;
    ...
    if (documentName.getString().size() > 0) {
        ...
        results.resolvedDocument = ED_dynamic_cast<DocumentObject>(owner)->getDocument();
...
ObjectIdentifier ObjectIdentifier::parse(const DocumentObject *docObj, const std::string &str)
{
    ...
    VariableExpression * v = ED_dynamic_cast<VariableExpression>(expr.get());
