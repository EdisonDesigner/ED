...
Expression * OperatorExpression::eval() const
{
    ...
    v1 = ED_dynamic_cast<NumberExpression>(e1.get());
    v2 = ED_dynamic_cast<NumberExpression>(e2.get());
...
Expression *OperatorExpression::simplify() const
{
    ...
    if (ED_dynamic_cast<NumberExpression>(v1) && ED_dynamic_cast<NumberExpression>(v2)) {
        delete v1;
        delete v2;
        return eval();
    }
...
std::string OperatorExpression::toString() const
{
    ...
    needsParens = false;
    if (ED_dynamic_cast<OperatorExpression>(left))
    ...
    needsParens = false;
    if (ED_dynamic_cast<OperatorExpression>(right))
    ...
    for (size_t i = 0; i< args.size(); ++i) {
        if (args[i]->isDerivedFrom(RangeExpression::getClassTypeId())) {
            ...
            do {
                ...
                if ((qp = ED_dynamic_cast<PropertyQuantity>(p)) != 0)
                    c->collect(qp->getQuantityValue());
                else if ((fp = ED_dynamic_cast<PropertyFloat>(p)) != 0)
            ...
            NumberExpression * n(ED_dynamic_cast<NumberExpression>(e.get()));
...
Expression * FunctionExpression::eval() const
{
    ...
    NumberExpression * v1 = ED_dynamic_cast<NumberExpression>(e1.get());
    NumberExpression * v2 = ED_dynamic_cast<NumberExpression>(e2.get());
    double output;
    Unit unit;
    double scaler = 1;
...
Expression *FunctionExpression::simplify() const
{
    ...
    if (ED_dynamic_cast<NumberExpression>(v1)) {
        switch (f) {
        case ATAN2:
        case MOD:
        case POW: {
            ...
            if (ED_dynamic_cast<NumberExpression>(v2)) {
                delete v1;
                delete v2;
                return eval();
            }
...
Expression *ConditionalExpression::eval() const
{
    ...
    NumberExpression * v = ED_dynamic_cast<NumberExpression>(e.get());
...
UnitExpression * ExpressionParser::parseUnit(const App::DocumentObject *owner, const char* buffer)
{
    ...
    if (!unitExpression) {
        OperatorExpression * fraction = ED_dynamic_cast<OperatorExpression>(ScanResult);

        if (fraction && fraction->getOperator() == OperatorExpression::DIV) {
            NumberExpression * nom = ED_dynamic_cast<NumberExpression>(fraction->getLeft());
            UnitExpression * denom = ED_dynamic_cast<UnitExpression>(fraction->getRight());
            const double epsilon = std::numeric_limits<double>::epsilon();
    ...
    if (unitExpression) {
        NumberExpression * num = ED_dynamic_cast<NumberExpression>(simplified);

        if (num) {
           simplified = new UnitExpression(num->getOwner(), num->getQuantity());
            delete num;
        }
        return ED_dynamic_cast<UnitExpression>(simplified);
    }
    else {
        delete simplified;
