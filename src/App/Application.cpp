...
# include <sstream>
# include <exception>
# if defined(ED_OS_LINUX) || defined(ED_OS_MACOSX) || defined(ED_OS_BSD)
# include <unistd.h>
# include <pwd.h>
# include <sys/types.h>
...
BaseExport extern PyObject* Base::BaseExceptionEdisonDesignerError;
  ...
  : _mConfig(mConfig), _pActiveDoc(0)
{
    ...
    // Python exceptions
    Base::BaseExceptionEdisonDesignerError = PyErr_NewException("Base.EDError", PyExc_RuntimeError, NULL);
	Py_INCREF(Base::BaseExceptionEdisonDesignerError);
	PyModule_AddObject(pBaseModule, "EDError", Base::BaseExceptionEdisonDesignerError);
...
void Application::ParseOptions(int ac, char ** av)
{
    ...
    if (vm.count("help")) {
        ...
        str << "For detailed descripton see http://edisondesigner.tistory.com" << endl<<endl;
...
#if defined(ED_OS_LINUX) || defined(ED_OS_CYGWIN) || defined(ED_OS_BSD)
