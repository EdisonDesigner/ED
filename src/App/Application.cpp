...
BaseExport extern PyObject* Base::BaseExceptionEDError;
...
PyDoc_STRVAR(ED_doc,
     "The functions in the ED module allow working with documents.\n"
     "The ED instance provides a list of references of documents which\n"
  ...
  : _mConfig(mConfig), _pActiveDoc(0)
{
    ...
    PyObject* pAppModule = Py_InitModule3("ED", Application::Methods, ED_doc);
    Py::Module(pAppModule).setAttr(std::string("ActiveDocument"),Py::None());

    PyObject* pConsoleModule = Py_InitModule3("__EDConsole__", ConsoleSingleton::Methods, Console_doc);
    ...
    // remove these types from the ED module.
    PyObject* pBaseModule = Py_InitModule3("__EDBase__", NULL,
        "The Base module contains the classes for the geometric basics\n"
        "like vector, matrix, bounding box, placement, rotation, axis, ...");

    // Python exceptions
    Base::BaseExceptionEDError = PyErr_NewException("Base.EDError", PyExc_RuntimeError, NULL);
    Py_INCREF(Base::BaseExceptionEDError);
    PyModule_AddObject(pBaseModule, "EDError", Base::BaseExceptionEDError);
...
Document* Application::newDocument(const char * Name, const char * UserName)
{
    ...
    // make sure that the active document is set in case no GUI is up
    {
        ...
        Py::Module("ED").setAttr(std::string("ActiveDocument"),active);
    }
...
void Application::setActiveDocument(Document* pDoc)
{
    ...
    if (pDoc) {
        ...
        Base::PyGILStateLocker lock;
        Py::Module("ED").setAttr(std::string("ActiveDocument"),Py::None());
    }
...
void Application::addImportType(const char* Type, const char* ModuleName)
{
    ...
    // Due to branding stuff replace "ED" with the branded application name
    if (strncmp(Type, "ED", 7) == 0) {
 ...
 */
#ifdef _MSC_VER // New handler for Microsoft Visual C++ compiler
int __cdecl EDNewHandler(size_t size )
{
    // throw an exception
...
#ifdef _MSC_VER // Microsoft compiler
        _set_new_handler ( EDNewHandler ); // Setup new handler
        _set_new_mode( 1 ); // Re-route malloc failures to new handler !
#else   // Ansi compiler
        std::set_new_handler (EDNewHandler); // ANSI new handler
#endif
...
void Application::initConfig(int argc, char ** argv)
{
    ...
    if (App::Application::Config().find("BuildVersionMajor") == App::Application::Config().end()) {
        std::stringstream str; str << EDVersionMajor << "." << EDVersionMinor;
        App::Application::Config()["ExeVersion"         ] = str.str();
        App::Application::Config()["BuildVersionMajor"  ] = EDVersionMajor;
        App::Application::Config()["BuildVersionMinor"  ] = EDVersionMinor;
        App::Application::Config()["BuildRevision"      ] = EDRevision;
        App::Application::Config()["BuildRepositoryURL" ] = EDRepositoryURL;
        App::Application::Config()["BuildRevisionDate"  ] = EDRevisionDate;
#if defined(EDRepositoryHash)
        App::Application::Config()["BuildRevisionHash"  ] = EDRepositoryHash;
#endif
#if defined(EDRepositoryBranch)
        App::Application::Config()["BuildRevisionBranch"] = EDRepositoryBranch;
#endif
    }
...
void Application::initApplication(void)
{
    ...
    // register scripts
    new ScriptProducer( "EDInit",    EDInit    );
    new ScriptProducer( "EDTest",    EDTest    );
    ...
    Interpreter().runString(Base::ScriptFactory().ProduceScript("EDInit"));
}
...
void Application::runApplication()
{
    ...
    if (mConfig["RunMode"] == "Cmd") {
        ...
        Interpreter().runCommandLine("ED Console mode");
    }
    ...
    try {
        if (_pcSysParamMngr->LoadOrCreateDocument(mConfig["SystemParameter"].c_str()) && !(mConfig["Verbose"] == "Strict")) {
            ...
            if (!Py_IsInitialized()) {
                ...
                Console().Message("   This warning normally means that ED is running for the first time\n"
                                  "   or the configuration was deleted or moved. ED is generating the standard\n"
                                  "   configuration.\n");
            }
        }
    }
...
void Application::ExtractUserPath()
{
	...
	mConfig["DataPath"] = mConfig["AppHomePath"] + "Data" + PATHSEP;
...
std::string Application::FindHomePath(const char* sCall)
{
	...
	// import the ED.pyd module from a running Python session. In the latter case the
	...
	// 2016. 11. 26 modified by YK
	// szFileName에서 "Bin"단어를 찾아 하위 문자열을 삭제
	// pos값은 Bin단어의 시작 포인트를 가르킴
	std::wstring::size_type pos = Call.find(L"Bin");
	...
	// fixes #0001638 to avoid to load DLLs from Windows' system directories before ED's bin folder
