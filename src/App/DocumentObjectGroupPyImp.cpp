...
PyObject*  DocumentObjectGroupPy::newObject(PyObject *args)
{
    ...
    if ( object ) {
        ...
        PyErr_Format(Base::BaseExceptionEdisonDesignerError, "Cannot create object of type '%s'", sType);
        return NULL;
    }
}
...
PyObject*  DocumentObjectGroupPy::addObject(PyObject *args)
{
    ...
    if (!docObj->getDocumentObjectPtr() || !docObj->getDocumentObjectPtr()->getNameInDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot add an invalid object");
        return NULL;
    }
    if (docObj->getDocumentObjectPtr()->getDocument() != getDocumentObjectGroupPtr()->getDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot add an object from another document to this group");
        return NULL;
    }
    if (docObj->getDocumentObjectPtr() == this->getDocumentObjectGroupPtr()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot add a group object to itself");
        return NULL;
    }
    if (docObj->getDocumentObjectPtr()->getTypeId().isDerivedFrom(DocumentObjectGroup::getClassTypeId())) {
        ...
        if (this->getDocumentObjectGroupPtr()->isChildOf(docGrp)) {
            PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot add a group object to a child group");
            return NULL;
        }
    }
...
PyObject*  DocumentObjectGroupPy::removeObject(PyObject *args)
{
    ...
    DocumentObjectPy* docObj = static_cast<DocumentObjectPy*>(object);
    if (!docObj->getDocumentObjectPtr() || !docObj->getDocumentObjectPtr()->getNameInDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot remove an invalid object");
        return NULL;
    }
    if (docObj->getDocumentObjectPtr()->getDocument() != getDocumentObjectGroupPtr()->getDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot remove an object from another document from this group");
        return NULL;
    }
...
PyObject*  DocumentObjectGroupPy::hasObject(PyObject *args)
{
    ...
    DocumentObjectPy* docObj = static_cast<DocumentObjectPy*>(object);
    if (!docObj->getDocumentObjectPtr() || !docObj->getDocumentObjectPtr()->getNameInDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot check an invalid object");
        return NULL;
    }
    if (docObj->getDocumentObjectPtr()->getDocument() != getDocumentObjectGroupPtr()->getDocument()) {
        PyErr_SetString(Base::BaseExceptionEdisonDesignerError, "Cannot check an object from another document with this group");
        return NULL;
    }
