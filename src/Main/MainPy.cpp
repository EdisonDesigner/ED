...
extern "C"
{
    void MainExport initED() {
...
#elif defined(ED_OS_CYGWIN)
        HMODULE hModule = GetModuleHandle("ED.dll");
        char szFileName [MAX_PATH];
        ...
        Dl_info info;
        int ret = dladdr((void*)initED, &info);
        if ((ret == 0) || (!info.dli_fname)) {
            PyErr_SetString(PyExc_ImportError, "Cannot get path of the ED module!");
            return;
        }
