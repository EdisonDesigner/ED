...
EDGui_setupWithoutGUI(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!Gui::Application::Instance) {
        ...
        PyErr_SetString(PyExc_RuntimeError, "EDGui already initialized");
        return 0;
    }
...
EDGui_embedToWindow(PyObject * /*self*/, PyObject *args)
{
    ...
    if (!widget) {
        PyErr_SetString(Base::BaseExceptionEDError, "No main window");
        return 0;
    }
    ...
    if (!Gui::MainWindow::getInstance()) {
        ...
        if (!init) {
            try {
                ...
                PyErr_Format(Base::BaseExceptionEDError, "Error in EDGuiInit.py: %s\n", e.what());
                return 0;
            }
            init = true;
        }
...
PyMODINIT_FUNC initEDGui()
{
    try {
        ...
        App::Application::Config()["AppIcon"] = "ED";
        App::Application::Config()["SplashScreen"] = "EDsplash";
        App::Application::Config()["CopyrightInfo"] = "\xc2\xa9 Youngki Kim 2014-2016, based on the FreeCAD\n";
