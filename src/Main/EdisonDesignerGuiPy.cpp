    ...
    if (!Gui::MainWindow::getInstance()) {
        ...
        if (!init) {
            try {
                ...
                Base::Interpreter().runString(Base::ScriptFactory().ProduceScript("EdisonDesignerGuiInit"));
            }
            catch (const Base::Exception& e) {
                PyErr_Format(Base::BaseExceptionFreeCADError, "Error in EdisonDesignerGuiInit.py: %s\n", e.what());
                return 0;
            }
            init = true;
        }
...
PyMOD_INIT_FUNC(FreeCADGui)
{
    try {
        ...
        App::Application::Config()["CopyrightInfo"] = "\xc2\xa9 Youngki Kim, Chiho Noh | iCAD Lab. in KAIST 2014-2018\n";
